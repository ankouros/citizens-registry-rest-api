...
### 1.7.1 Σημείωση για διόρθωση CI (citizen-it)

Κατά τη λειτουργία του GitHub CI εντοπίστηκε σφάλμα μεταγλώττισης στο module
`citizen-it`, επειδή η κλάση `CitizenApplication` δεν ήταν διαθέσιμη στο
classpath των integration tests. Η αιτία ήταν ότι η εξάρτηση `citizen-service`
είχε δηλωθεί με `scope` **test**, με αποτέλεσμα να μην είναι ορατή κατά το
`test-compile` του `citizen-it`.

Η διόρθωση που εφαρμόστηκε ήταν η **αφαίρεση** του `scope` ώστε η εξάρτηση να
είναι διαθέσιμη στο compile classpath του `citizen-it`, διατηρώντας αμετάβλητη
τη λειτουργική συμπεριφορά των tests, αλλά αποκαθιστώντας την επιτυχή εκτέλεση
του CI.

Κατά την επανεκτέλεση του CI, το ίδιο σφάλμα επανεμφανίστηκε, επειδή το
`citizen-service` δημοσιευόταν ως Spring Boot **repackaged** (fat) JAR, στο οποίο
οι κλάσεις μεταφέρονται στο `BOOT-INF/classes` και δεν είναι ορατές στο
`test-compile`. Η διόρθωση ήταν **η παραγωγή εκτελέσιμου JAR με classifier**
(`exec`) από το `spring-boot-maven-plugin`, ώστε να διατηρείται **κανονικό**
JAR για εξαρτήσεις, ενώ το Docker χρησιμοποιεί το `*exec.jar` ως runtime
artefact.
