...
- **Multi-stage Dockerfile** με Maven build και JRE runtime, υποστήριξη multi-module build
  (συμπεριλαμβάνεται το `citizen-domain`).
- **Μη-ριζικό χρήστη** για την εκτέλεση της υπηρεσίας, ώστε να μειώνεται το ρίσκο.
...
- **Βασικές πρακτικές ασφάλειας**: read-only filesystem, `no-new-privileges`,
  απουσία έκθεσης θυρών της βάσης στο τοπικό σύστημα.
...
Το έργο **Citizen Registry REST API** αποτελεί ένα ολοκληρωμένο και τεκμηριωμένο
παράδειγμα σύγχρονης ανάπτυξης λογισμικού με Spring Boot, σχεδιασμένο ώστε να
ικανοποιεί πλήρως τις απαιτήσεις των ενοτήτων:

- **l3687**: Σχεδίαση και υλοποίηση RESTful υπηρεσιών με πολυεπίπεδη αρχιτεκτονική
- **l3688**: Συνεχής Ενοποίηση και Παράδοση (CI/CD) με GitHub Actions
- **l3689 – l3691**: Προετοιμασία για cloud deployment, containerisation και orchestration

Η υλοποίηση συνοδεύεται από πλήρη κάλυψη δοκιμών (unit, ORM, integration),
καθαρή οργάνωση Maven και αναπαραγώγιμη διαδικασία CI/CD.

Το παρόν README λειτουργεί τόσο ως τεχνική τεκμηρίωση
πανεπιστημιακού επιπέδου όσο και ως βάση για περαιτέρω επεκτάσεις
στις επόμενες ενότητες του μαθήματος.
